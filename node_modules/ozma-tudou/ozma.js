...
var uglify_jsp = uglifyjs.parser;
var uglify_pro = uglifyjs.uglify;
...
var RE_REQUIRE_TEXT_VAL = /(\b)require\.text\(\s*(['"])(.+?)\2\s*\)/g;
...
function Ozma(){
    ...
    Oz.exec = function(list){
        ...
        // Replace require.text(uri)
        output_code = output_code.replace(RE_REQUIRE_TEXT_VAL, function(all, prefix, quote, uri) {
            var filePath = path.join(_config.baseUrl, uri);
            if (!fs.existsSync(filePath)) {
                logger.log(INDENTx1, '\033[33m' + 'File not found: ', filePath + '\033[0m');
                return all;
            }
        ...
        //output_code = output_code.replace(/^( * @(?:url|modified|version) )\$((?:URL|Author|Rev):\s*[^\$\s]\s*)\$$/gm, '$1$2');

        output_code = BANNER + output_code;
        ...
        writeFile3721(output, output_code, function(err){
            ...
            // create min file
            var ast = uglify_jsp.parse(output_code); // parse code and get the initial AST
            ast = uglify_pro.ast_mangle(ast);
            ast = uglify_pro.ast_squeeze(ast);
            var min_output_code = uglify_pro.gen_code(ast) + ';'; // compressed code

            min_output_code = BANNER + min_output_code;

            var min_output = output.replace(/(^|[\/\\])build([\/\\])js([\/\\])/, '$1dist$2js$3');

            writeFile3721(min_output, min_output_code, function(err){
                if (err) {
                    throw err;
                }
                logger.log(INDENTx1, 'target: ', '\033[4m' + min_output + '\033[0m');
                logger.log(INDENTx1, 'Success!\n');
                if (_complete_callback) {
                    _complete_callback(output, min_output);
                }
                _output_count++;
